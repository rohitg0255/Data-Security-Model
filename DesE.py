...
def InputToBin(x): #input to binary
    BinStr=''.join(format(ord(i),'08b') for i in x)
    return BinStr
def BinChunks(): #binary to 64 bit data chunks
    data=InputToBin(text)
    chunks=[data[i:i+64] for i in range(0, len(data), 64)]
    if (len(chunks[-1])!=64):
        c=chunks[-1]+''.join('0' for i in range(0,64-len(chunks[-1])))
    chunks.pop(-1)
    chunks.append(c)
    return(chunks)# class list->class str
def InitialPermutation():#ip of 64 bit data
    data=BinChunks()
    ipchunks=[]
    for i in data:
        ipdata=''.join(str(i[k-1]) for k in IP)
        ipchunks.append(ipdata)
    return(ipchunks)   
def SessionKey(StringLength=8): #generates sessionkey
    """Generate a random string of letters, digits and special characters"""
    password_characters = string.ascii_letters + string.digits + string.punctuation
    generated_key=''.join(random.choice(password_characters) for i in range(StringLength))
    with open('sessionkey.txt','w') as s:
        s.write(generated_key)
...
def IP_reverseTransp(K): #final permutation of data
    ...
    return cipher        
def generateHex(reversedP): # generate hex for cipher
        reversedP_str=''.join(str(i) for i in reversedP)
        chunk = [reversedP_str[i:i+4] for i in range(0, len(reversedP_str), 4)]
        hexd=[hex(int(chun, 2))[2:] for chun in chunk]
        cipher=''.join(str(hex) for hex in hexd)
        return(cipher)
def RsaCipher():
    ...
    with open ('public.txt','r') as pu:
        ...
        e=int(pu.readline())
    publickey=(n,e)
    return Rsa.EncryptSessionKey(publickey, sessionkey)
def FuncEncryption():
        data=BinChunks()
        K=GenerateKset()
        cip=[]
        for dat in data:
            ip=''.join(str(dat[k-1]) for k in IP)
            L=ip[:32]
            R=ip[32:]
            for i in range(15):
                newR=xor(L,Feistel(R,K[i]))
                L=R
                R=newR
            p_result = Feistel(R,K[15])
            L = xor(L,p_result)   
            FinalPermutation=IP_reverseTransp(L+R)
            cipher=generateHex(FinalPermutation)
            cip.append(cipher)
        cipher=''.join(c for c in cip) 
        with open('cipher.txt','w') as c:
            c.write(cipher)
        key_cipher=RsaCipher()
        with open('ciphersessionkey.txt','w') as s:
            for item in key_cipher:
                s.write("%s\n" %item)
